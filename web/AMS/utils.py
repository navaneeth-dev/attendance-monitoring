...
def fetch_att(username, pwd, max_retries=3):
    for attempt in range(max_retries):
        with sync_playwright() as p:
            browser = p.chromium.launch(headless=os.getenv("DEBUG", "true") == "false")
            ...
            profile.goto(
                "http://184.95.52.42/velsonline/students/template/PageLeft.jsp"
            )
            profile.wait_for_selector("td.ui-state-highlight.ui-corner-all b")
            student_name = profile.locator(
                "td.ui-state-highlight.ui-corner-all b"
            ).inner_text()

            # Navigate to attendance page and fetch percentage and end date
            att_frame = context.new_page()
            att_frame.goto(
                "http://184.95.52.42/velsonline/students/report/studentSubjectWiseAttendance.jsp"
            )
            att_frame.wait_for_selector(
                '#tblSubjectWiseAttendance tr.subtotal td:has-text("%")'
            )
            percentage = att_frame.locator(
                '#tblSubjectWiseAttendance tr.subtotal td:has-text("%")'
            ).inner_text()
            end_date = att_frame.locator(
                "#tblSubjectWiseAttendance tr.subheader1 td:nth-child(4)"
            ).inner_text()

            subs = att_frame.locator(
                "#tblSubjectWiseAttendance > tbody td:nth-child(2)"
            ).all_inner_texts()
            subs = subs[2:-1]

            sub_percents = att_frame.locator(
